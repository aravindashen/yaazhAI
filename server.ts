...
} from './src/services/elevenlabsService.ts';
...
app.post('/api/ask', async (req, res) => {
  try {
    ...
    const { language = 'ta', languageMode, currentRole = 'study', history = [] } = req.body;
    ...
    // Determine effective language mode ('ta' | 'en' | 'tanglish')
    let effectiveLangMode: 'ta' | 'en' | 'tanglish' = languageMode || (language === 'en' ? 'en' : 'ta');
    if (/(tanglish\s*(la|il|la\s*sollu|la\s*explain|le)|speak\s*tanglish|in\s*tanglish)/i.test(lowerQuery)) {
      effectiveLangMode = 'tanglish';
    } else if (/(tamil\s*(la|il|la\s*sollu|la\s*explain)|speak\s*tamil|in\s*tamil|தமிழில்\s*(விளக்கு|சொல்|கூறு))/i.test(lowerQuery)) {
      effectiveLangMode = 'ta';
    } else if (/(english\s*(la|il|la\s*sollu|la\s*explain)|speak\s*english|in\s*english|ஆங்கிலத்தில்\s*(விளக்கு|சொல்))/i.test(lowerQuery)) {
      effectiveLangMode = 'en';
    }

    const isTa = effectiveLangMode === 'ta';
    ...
    const isTranslateIntent = !isTopicQuery && (
      ...
      /\btranslate\s+.+\s+(to|into)\s+(english|tamil|tanglish)\b/i.test(cleanQuery) ||
...
Provide a JSON response with:
{
  ...
  "transliteration": "ISO 15919 transliteration"
    ...
    if (ai) {
      try {
        let langName = 'Tamil (தமிழ்)';
        let langSpecificRules = '';
        if (effectiveLangMode === 'tanglish') {
          langName = 'Tanglish (Romanized Tamil script)';
          langSpecificRules = `LANGUAGE MODE: TANGLISH.
- Provide the explanation, word meanings, and commentary in natural, fluent Tanglish (Romanized Tamil words written in English alphabet, e.g., "Intha kural la Valluvar solla varadhu enna na...", "Yadhum oore yaavarum kelir na ellarum namma sondham nu artham...").
- Keep any Classical Tamil verses or poem lines in their authentic, uncorrupted Tamil script (along with romanized transliteration) so the original literature remains pristine.`;
        } else if (effectiveLangMode === 'en') {
          langName = 'English';
          langSpecificRules = `LANGUAGE MODE: ENGLISH.
- Provide a clear, scholarly, and fluent English explanation.
- Include original Classical Tamil keywords and citations accurately.`;
        } else {
          langName = 'Tamil (தமிழ்)';
          langSpecificRules = `LANGUAGE MODE: TAMIL.
- Provide authoritative, lucid, and rich Classical and Modern Tamil scholarly prose.`;
        }
...
2. If the user asks "Read this poem" or "பாடலை வாசி", recite the original poem lines in Tamil with their meter and poignant meaning.
3. If the user asks "Explain the difficult words" or "கடின சொற்கள்", break down the key vocabulary, sandhi splits, and meanings step-by-step.
4. If the user asks "Give me a quiz" or "வினாடி வினா", provide a thought-provoking 4-option quiz with question, options, correct answer, and explanation.
5. If the user asks about Tamil literature, poetry, or ethics, reference specific works (Sangam, Tolkappiyam, Tirukkural, Silappadikaram, etc.), verses, authors, and provide clear explanations.
6. Use clean formatting with paragraphs, markdown bold highlights, and bullet points where helpful.
7. Suggest 2-3 natural follow-up questions to help the user learn more.
...
// ==========================================
// ElevenLabs AI Voice & Language Endpoints
// ==========================================

// 1. ElevenLabs Voice Status & Capabilities
app.get('/api/voice/status', (req, res) => {
  const configured = isElevenLabsConfigured();
  res.json({
    elevenlabsConfigured: configured,
    voices: DEFAULT_ELEVENLABS_VOICES,
    supportedLanguages: ['ta', 'en', 'tanglish'],
    capabilities: {
      speechToText: configured,
      textToSpeech: configured,
      tanglishNormalization: true,
      voiceAssistant: true
    }
  });
});

// 2. ElevenLabs Speech-to-Text (STT) Endpoint
app.post('/api/voice/stt', async (req, res) => {
  try {
    const { audioBase64, mimeType = 'audio/webm', languageCode } = req.body;
    ...
    if (audioBuffer.length > 15 * 1024 * 1024) {
      return res.status(400).json({ error: 'Audio file exceeds 15MB limit.' });
    }

    if (!isElevenLabsConfigured()) {
      if (ai) {
        try {
          const geminiPrompt = languageCode === 'ta'
            ? 'Transcribe this spoken Tamil audio verbatim into accurate Tamil script (or Tanglish if mixed). Output ONLY the transcribed words, without explanations or quotation marks.'
            : languageCode === 'en'
              ? 'Transcribe this spoken English audio verbatim. Output ONLY the transcribed words.'
              : 'Transcribe this spoken Tamil, English, or Tanglish audio verbatim. Output ONLY the transcribed words, without any explanations or markdown.';
          ...
          if (transcribed) {
            const norm = normalize_tanglish(transcribed);
            return res.json({
              success: true,
              text: transcribed,
              ...
              detectedCommand: norm.detectedCommand,
              isCommand: norm.isCommand,
              provider: 'gemini_multimodal'
            });
          }
        } catch (geminiAudioErr) {
          console.warn('Gemini audio transcription fallback failed:', geminiAudioErr);
        }
      }
      ...
      return res.status(503).json({
        ...
        message: 'ElevenLabs API key is not configured in backend environment variables.',
        fallback: 'browser'
      });
    }

    // Map language selection to ElevenLabs language_code
    let targetLangCode: string | undefined = undefined;
    if (languageCode === 'ta' || languageCode === 'tam') targetLangCode = 'tam';
    else if (languageCode === 'en' || languageCode === 'eng') targetLangCode = 'eng';
    // For Tanglish or auto, leave undefined so Scribe auto-detects multilingual phonetics

    const sttResult = await speech_to_text(audioBuffer, mimeType, {
      languageCode: targetLangCode
    });

    const norm = normalize_tanglish(sttResult.text);
...
// 3. ElevenLabs Text-to-Speech (TTS) Endpoint
app.post('/api/voice/tts', async (req, res) => {
  try {
    const { text, voiceId, language = 'ta', stability, similarityBoost } = req.body;

    if (!text || typeof text !== 'string' || !text.trim()) {
      return res.status(400).json({ error: 'Valid text is required for Text-to-Speech.' });
    }
    ...
    const ttsResult = await text_to_speech(text, {
      voiceId,
      language: language as 'ta' | 'en' | 'tanglish',
      stability,
      similarityBoost
    });
...
// 4. Tanglish & Language Normalization Endpoint
app.post('/api/voice/normalize', (req, res) => {
  try {
    const { text } = req.body;
    if (!text || typeof text !== 'string') {
      return res.status(400).json({ error: 'Valid text string is required for normalization.' });
    }
    const result = normalize_tanglish(text);
    res.json(result);
  } catch (error: any) {
    res.status(500).json({ error: 'Normalization failed', message: error.message });
  }
});
...
app.post('/api/voice/process', async (req, res) => {
  try {
    ...
    // 3.5 Language Switching Voice Commands
    if (/(tanglish\s*(la|il|la\s*sollu|la\s*explain|le)|speak\s*tanglish|in\s*tanglish)/i.test(lower)) {
      return res.json({
        intent: 'SET_LANGUAGE',
        languageMode: 'tanglish',
        speechText: 'Ini ungalukku Tanglish la badhil solren. Enna kavidhai pathi theriya vendum?',
        displayText: 'Language switched to Tanglish (தமிங்கிலம்). இனி விளக்கங்கள் டாங்கிலிஷில் வழங்கப்படும்.',
        verse: activeVerse,
        followUpSuggestions: ['Intha kural explain pannu', 'Difficult words sollu', 'Quiz kudu']
      });
    }

    if (/(tamil\s*(la|il|la\s*sollu|la\s*explain)|speak\s*tamil|தமிழில்\s*(விளக்கு|சொல்|கூறு))/i.test(lower)) {
      return res.json({
        intent: 'SET_LANGUAGE',
        languageMode: 'ta',
        speechText: 'இனி பதில்கள் தூய தமிழில் வழங்கப்படும். நீங்கள் என்ன அறிய விரும்புகிறீர்கள்?',
        displayText: 'Language switched to Tamil (தமிழ்). இனி விளக்கங்கள் தமிழில் வழங்கப்படும்.',
        verse: activeVerse,
        followUpSuggestions: ['இப்பாடலின் எளிய விளக்கம்', 'கடின சொற்கள்', 'வினாடி வினா']
      });
    }

    if (/(english\s*(la|il|la\s*sollu|la\s*explain)|speak\s*english|explain\s*in\s*english|ஆங்கிலத்தில்\s*(விளக்கு|சொல்))/i.test(lower)) {
      return res.json({
        intent: 'SET_LANGUAGE',
        languageMode: 'en',
        speechText: 'Language switched to English. How can YAAZH help you explore Classical Tamil literature today?',
        displayText: 'Language switched to English. Responses will be provided in English.',
        verse: activeVerse,
        followUpSuggestions: ['Explain this poem', 'Explain difficult words', 'Give me a quiz']
      });
    }
